...
def mm_to_mils(mm):
    return float(mm * 39.37)

def mils_to_mm(mils):
    return float(mils / 39.37)
...
def test_netclass(netclass, tolerance, nets):
    ...
    print_color(BRIGHTGREEN if meets else BRIGHTRED,
                "%s -> %s TOLERANCE %.2f (max variance %.2f)" % (netclass, "MEETS" if meets else "FAILS", mm_to_mils(nm_to_mm(tolerance)), mm_to_mils(nm_to_mm(delta))))
